...
# second layer: jitter points of raw data   
  ...
  labs(title = "How Water Intake Compares to the Type of Workout",  # adding title
...
```{r Calculate Summary Stats for Table}
...
When I first thought about affective data visualization, I was drawn to the idea of humanizing numbers on a spreadsheet into something more tangible and relatable. For my project, I plan to represent average water intake by illustrating body parts like back, arms, chest, legs, abdomen, and shoulders along the x-axis. Instead of traditional bars or box plots, each muscle group will be visually represented and "filled" with water to reflect the average amount consumed during workouts targeting that area. To complement this, a small flame icon above each body part will indicate average workout intensity, with four levels/rings that represent the average level of intensity for that body part (more rings filled = higher average intensity). This approach personalizes the data, highlighting the physical connection between hydration and strain while making the visualization emotionally engaging and easy to interpret.
...
![Water intake (liters) by workout type](Draftdata.jpg)
...
The influences:
The inspiration came from the lecture examples, particularly the violin plots, which sparked the idea of representing each muscle group as a visual shape. From there, I developed the concept of filling each body part with a water level to reflect hydration while adding in flame icons to communicate workout intensity in an emotionally resonant way.

The form of your work:
The piece is a digital drawing that combines artistic illustration with data-informed storytelling, allowing for customized body part silhouettes enhanced with color and symbolic elements to convey both quantitative (water intake) and qualitative (intensity) data. This format provides a clear and expressive way to represent personal metrics, which can be reused as a fun way to track future trends.

The process:
I was initially confronted with a creative block, and I needed to take a step back to reexamine my data, not just as numbers, but as a reflection of physical effort, which led me to abandon traditional charting and instead visualize the body as a site of intensity. This reframing allowed me to design an illustration where hydration and exertion are mapped directly onto the body, transforming abstract data into something personal. 
