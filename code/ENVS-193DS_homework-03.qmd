...
data_summary <- data_clean |> # starting data frame 
  ...
  summarize(
    ...
    median = median(water_consumption), # median calculation 
    lower = quantile(water_consumption, 0.25), # lower quantile calculation 
    upper = quantile(water_consumption, 0.75), # upper quantile calculation 
    minimum = min(water_consumption), # minimum value calculation 
    maximum = max(water_consumption)) |> # maximum value calculation 
  mutate(
...
data_summary |> # starting data frame that has summary statistics
  ...
  tab_header(title = "Summary Statistics of Water Intake by Workout Type") |> # creating table header/title
